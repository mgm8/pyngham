...
class PyNGHamSPP:
    ...
    def encode(self, pkt_type, pl):
        ...
        crc_val = Calculator(Configuration(16, 0x1021, 0xFFFF, 0xFFFF, True, True)).calculate_checksum(pkt)
    ...
    def decode_byte(self, c):
        if self._state == SPPState.START:
            ...
            if len(self._rx_buffer) == (2 + 1 + 1 + self._rx_buffer[3]):
                # Check checksum
                crc_val = Calculator(Configuration(16, 0x1021, 0xFFFF, 0xFFFF, True, True)).calculate_checksum(self._rx_buffer[2:])
